...
export const saveActivityLogsNotification = async ({
  ...
  //if user is being created for the first time, then new user entry will not be there in db, in this case you can just find first a user in the agency and create the save the activity notification
  if (!authUser) {
  ...
  if (!foundAgencyId) {
    ...
    //find agency id using subaccountid
...
export const verifyAndAcceptInvitation = async () => {
  ...
  if (invitationExists) {
    ...
    return userInDb ? userInDb.agencyId : null;
  }
};
...
export const upsertSubAccount = async (subAccount: SubAccount) => {
  ...
  if (!agencyOwner) {
    return console.log('Error could not createe subaccount');
  }
